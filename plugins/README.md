...
This directory contains plugins for the Orbis plugin system.
...
let source = PluginSource::from_path(&PathBuf::from("plugins/hello-plugin/unpacked-external"))?;

// Unpacked with embedded manifest
let source = PluginSource::from_path(&PathBuf::from("plugins/hello-plugin/unpacked-embedded"))?;

// Standalone
let source = PluginSource::from_path(&PathBuf::from("plugins/hello-plugin/standalone.wasm"))?;

// Packed with external manifest
let source = PluginSource::from_path(&PathBuf::from("plugins/hello-plugin/packed-external.zip"))?;

// Packed with embedded manifest
let source = PluginSource::from_path(&PathBuf::from("plugins/hello-plugin/packed-embedded.zip"))?;
```
