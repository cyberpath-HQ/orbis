 ...
 */
export function PluginRenderer({ schema, data = {}, handlers = {} }: PluginRendererProps) {
 ...
 */
function ComponentRenderer({ schema, data, handlers }: ComponentRendererProps): React.ReactElement | null {
  // Check visibility
  ...
  switch (schema.type) {
    ...
    case 'Table':
      const tableData = getNestedValue(data, schema.dataSource) as Record<string, unknown>[] || [];
      return (
            ...
            {tableData.map((row, rowIndex) => (
              <tr key={schema.rowKey ? String(row[schema.rowKey]) : rowIndex}>
                {schema.columns.map((col) => (
                  <td key={col.key}>
                    {col.render ? (
                      <ComponentRenderer schema={col.render} data={{ ...data, row, rowIndex }} handlers={handlers} />
                    ) : (
                      String(row[col.key] ?? '')
                    )}
                  </td>
    ...
    case 'List':
      const listData = getNestedValue(data, schema.dataSource) as unknown[] || [];
        ...
        <ul {...baseProps} className={`orbis-list ${schema.className || ''}`}>
          {listData.map((item, index) => (
            <li key={index} className="orbis-list__item">
              <ComponentRenderer schema={schema.itemTemplate} data={{ ...data, item, index }} handlers={handlers} />
            </li>
          ))}
        </ul>
      );
    ...
    case 'Icon':
      return (
        <span {...baseProps} className={`orbis-icon orbis-icon--${schema.size || 'md'} ${schema.className || ''}`} style={{ color: schema.color, ...schema.style }}>
    ...
    case 'Progress':
      ...
      return (
        <div {...baseProps} className={`orbis-progress ${schema.className || ''}`}>
          <div className="orbis-progress__bar" style={{ width: `${percentage}%` }} />
    ...
    case 'Spacer':
      const spacerSizes = { xs: '0.25rem', sm: '0.5rem', md: '1rem', lg: '1.5rem', xl: '2rem' };
      return <div style={{ height: spacerSizes[schema.size], width: '100%' }} />;
...
function InputRenderer({ schema, data, handlers }: { schema: ComponentSchema & { type: 'Input' }; data: Record<string, unknown>; handlers: Record<string, (...args: unknown[]) => void> }) {
...
function TabsRenderer({ schema, data, handlers }: { schema: ComponentSchema & { type: 'Tabs' }; data: Record<string, unknown>; handlers: Record<string, (...args: unknown[]) => void> }) {
  const [activeTab, setActiveTab] = React.useState(schema.defaultTab || schema.items[0]?.key);
...
function AccordionRenderer({ schema, data, handlers }: { schema: ComponentSchema & { type: 'Accordion' }; data: Record<string, unknown>; handlers: Record<string, (...args: unknown[]) => void> }) {
