...
import { healthCheck, getMode, getPlugins, getPluginPages } from "./api";
import type { AppModeInfo, PluginInfo, PluginPage } from "./types";
import "./App.css";
import "./styles/plugin.css";

function App() {
  const [status, setStatus] = useState<"loading" | "connected" | "error">("loading");
  const [mode, setMode] = useState<AppModeInfo | null>(null);
  const [plugins, setPlugins] = useState<PluginInfo[]>([]);
  const [pluginPages, setPluginPages] = useState<PluginPage[]>([]);
  const [activePage, setActivePage] = useState<PluginPage | null>(null);
  ...
  useEffect(() => {
    async function init() {
      try {
        ...
        // Get plugins
        const { plugins: loadedPlugins } = await getPlugins();
