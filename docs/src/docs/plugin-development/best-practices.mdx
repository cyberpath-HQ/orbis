...
pub fn create_item(ctx: Context) -> Result<Response> {
    ...
    Ok(Response::json(&json!({ "item": item })))
}
```
</CodeBlock>
...
#[wrap_handler]
pub fn get_item(ctx: Context) -> Result<Response> {
...
// Good: Proper logging
#[wrap_handler]
pub fn delete_item(ctx: Context) -> Result<Response> {
    let id: i32 = ctx.query_param_as("id")?;
