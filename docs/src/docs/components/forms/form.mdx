...
```json
{
  ...
  "events": {
    "on_submit": [
      { "type": "call_api", "api": "auth.login", "args": "{{state.formData}}" }
    ]
  }
}
```
...
|-------|---------|---------|
| `on_submit` | Form submission | Form data |
| `on_change` | Any field change | `{ field, value }` |
    ...
    "on_submit": [
      {
        ...
        "args": "{{state.settings}}",
        "on_success": [
          { "type": "show_toast", "message": "Settings saved!", "level": "success" }
        ]
      }
    ]
  }
...
```json
{
  ...
  "events": {
    "on_submit": [
      { "type": "call_api", "api": "search", "args": "{{state.search}}" }
    ]
  }
}
```
  ...
  "children": [
    ...
    },
    {
      ...
      "then": {
        ...
        "events": {
          "on_submit": [
            { "type": "call_api", "api": "submit", "args": "{{state.wizard}}" }
          ]
        }
      }
    }
  ...
  "children": [
    {
      ...
      "variant": "outline",
      "events": {
        "on_click": [{ "type": "resetForm", "form": "myForm" }]
      }
    },
    {
      "type": "Button",
