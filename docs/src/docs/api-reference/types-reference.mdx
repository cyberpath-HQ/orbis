...
interface FieldSchema extends BaseComponentSchema {
  ...
  events?: {
    on_change?: ActionSchema[];
    on_blur?: ActionSchema[];
    on_focus?: ActionSchema[];
  };
}
...
interface CallApiAction {
  ...
  on_success?: ActionSchema[];
  on_error?: ActionSchema[];
      ...
      actions: [
        {
          ...
          text: 'Add User',
          events: {
            on_click: [{ type: 'navigate', route: '/users/new' }]
          }
        }
      ]
    },
